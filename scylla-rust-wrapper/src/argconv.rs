...
///
/// ## Why [`Mutability`] property is not enough?
...
/// From rust perspective, the pointer is always [`Const`], because it is shared behind an [`Arc`].
...
impl<M: Mutability, CM: CMutability> sealed::Sealed for (M, CM) {}
impl<M: Mutability, CM: CMutability> Properties for (M, CM) {
    type Mutability = M;
    type CMutability = CM;
}
...
impl<'a, T: Sized, P: Properties> CassPtr<'a, T, P> {
    pub fn into_c_const(self) -> CassPtr<'a, T, (P::Mutability, CConst)> {
        CassPtr {
            ptr: self.ptr,
...
impl<T: Sized> CassPtr<'_, T, (Mut, CMut)> {
    fn null_mut() -> Self {
        CassPtr {
            ptr: None,
            _phantom: PhantomData,
...
impl<'a, T: Sized, CM: CMutability> CassPtr<'a, T, (Mut, CM)> {
...
impl<T: Sized, P: Properties> CassPtr<'_, T, P> {
    ...
    pub fn borrow<'a>(&'a self) -> CassPtr<'a, T, (Const, P::CMutability)> {
        CassPtr {
            ptr: self.ptr,
            _phantom: PhantomData,
        }
...
impl<T: Sized> CassPtr<'_, T, (Mut, CMut)> {
    ...
    #[allow(clippy::needless_lifetimes)]
    pub fn borrow_mut<'a>(&'a mut self) -> CassPtr<'a, T, (Mut, CMut)> {
        CassPtr {
            ptr: self.ptr,
            _phantom: PhantomData,
        }
...
pub trait BoxFFI: Sized {
    ...
    fn from_ptr<M: Mutability, CM: CMutability>(
        ptr: CassPtr<'static, Self, (M, CM)>,
    ) -> Option<Box<Self>> {
        // SAFETY:
    ...
    #[allow(clippy::needless_lifetimes)]
    fn as_ref<'a, M: Mutability, CM: CMutability>(
        ptr: CassPtr<'a, Self, (M, CM)>,
    ) -> Option<&'a Self> {
        ptr.into_ref()
    }
    ...
    #[allow(clippy::needless_lifetimes)]
    fn as_mut_ref<'a, CM: CMutability>(ptr: CassPtr<'a, Self, (Mut, CM)>) -> Option<&'a mut Self> {
        ptr.into_mut_ref()
    }

    /// Frees the pointee.
    fn free<CM: CMutability>(ptr: CassPtr<'static, Self, (Mut, CM)>) {
        std::mem::drop(BoxFFI::from_ptr(ptr));
    }

    // Currently used only in tests.
    #[allow(dead_code)]
    fn null<'a, CM: CMutability>() -> CassPtr<'a, Self, (Const, CConst)> {
        CassPtr::null()
    }

    fn null_mut<'a>() -> CassPtr<'a, Self, (Mut, CMut)> {
        CassPtr::null_mut()
    }
}
...
pub trait ArcFFI: Sized {
    ...
    fn into_ptr<CM: CMutability>(self: Arc<Self>) -> CassPtr<'static, Self, (Const, CM)> {
        #[allow(clippy::disallowed_methods)]
    ...
    fn from_ptr<CM: CMutability>(ptr: CassPtr<'static, Self, (Const, CM)>) -> Option<Arc<Self>> {
        // SAFETY:
    ...
    fn cloned_from_ptr<CM: CMutability>(ptr: CassPtr<'_, Self, (Const, CM)>) -> Option<Arc<Self>> {
        // SAFETY:
    ...
    #[allow(clippy::needless_lifetimes)]
    fn as_ref<'a, CM: CMutability>(ptr: CassPtr<'a, Self, (Const, CM)>) -> Option<&'a Self> {
        ptr.into_ref()
    }

    /// Decreases the reference count (and potentially frees) of the owned pointer.
    fn free<CM: CMutability>(ptr: CassPtr<'static, Self, (Const, CM)>) {
        std::mem::drop(ArcFFI::from_ptr(ptr));
    }

    fn null<'a, CM: CMutability>() -> CassPtr<'a, Self, (Const, CM)> {
        CassPtr::null()
    }

    fn is_null<CM: CMutability>(ptr: &CassPtr<'_, Self, (Const, CM)>) -> bool {
        ptr.is_null()
    }
}
...
pub trait RefFFI: Sized {
    ...
    /// ```
    /// # use scylla_cpp_driver::argconv::{RefFFI, CConst, CassBorrowedConstPtr};
    /// # use std::sync::{Arc, Weak};
    ///
    /// struct Foo;
    /// impl RefFFI for Foo {}
    ...
    /// let ptr: CassBorrowedConstPtr<Foo, CConst> = unsafe { RefFFI::weak_as_ptr(&weak) };
    /// std::mem::drop(arc);
    ///
...
/// let borrowed_mut_ptr2: CassBorrowedMutPtr<Foo, CMut> = ptr.borrow_mut();
...
/// ```compile_fail,E0502
/// # use scylla_cpp_driver::argconv::{CassOwnedMutPtr, CassBorrowedConstPtr, CassBorrowedMutPtr, CConst, CMut};
/// # use scylla_cpp_driver::argconv::BoxFFI;
/// struct Foo;
/// impl BoxFFI for Foo {}
///
...
/// let borrowed_ptr: CassBorrowedConstPtr<Foo, CMut> = ptr.borrow();
...
/// ```compile_fail,E0505
/// # use scylla_cpp_driver::argconv::{CassOwnedMutPtr, CassBorrowedConstPtr, CMut};
/// # use scylla_cpp_driver::argconv::BoxFFI;
/// struct Foo;
/// impl BoxFFI for Foo {}
///
...
/// ```compile_fail,E0505
/// # use scylla_cpp_driver::argconv::{CassOwnedConstPtr, CassBorrowedConstPtr, CConst};
/// # use scylla_cpp_driver::argconv::ArcFFI;
/// # use std::sync::Arc;
/// struct Foo;
/// impl ArcFFI for Foo {}
///
/// let ptr: CassOwnedConstPtr<Foo, CConst> = ArcFFI::into_ptr(Arc::new(Foo));
/// let borrowed_ptr: CassBorrowedConstPtr<Foo, CConst> = ptr.borrow();
/// ArcFFI::free(ptr);
...
/// ```compile_fail,E0505
/// # use scylla_cpp_driver::argconv::{CassBorrowedConstPtr, CConst};
/// # use scylla_cpp_driver::argconv::ArcFFI;
/// # use std::sync::Arc;
/// struct Foo;
/// impl ArcFFI for Foo {}
///
/// let arc = Arc::new(Foo);
/// let borrowed_ptr: CassBorrowedConstPtr<Foo, CConst> = ArcFFI::as_ptr(&arc);
/// std::mem::drop(arc);
/// let immref = ArcFFI::cloned_from_ptr(borrowed_ptr);
/// ```
